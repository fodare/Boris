...
class LoginView(_tk.Frame):
    ...
    def render_login_frame(self, parent):
        ...
        self.password_entry = _tk.Entry(self, width=27).grid(
            row=2, column=1, columnspan=1)

        self.login_button = _tk.Button(
            self, text="Login", width=11).grid(row=3, column=0, pady=20,  rowspan=2)
...
class RegisterView(_tk.Frame):
    ...
    def render_register_frame(self, parent):
        ...
        self.login_button = _tk.Button(
            self, text="Login", width=11, command=handle_login_view).grid(row=3, column=0, pady=20,  rowspan=2)
        self.register_button = _tk.Button(
            self, text="Register", width=11).grid(row=3, column=1, pady=20, rowspan=2)
